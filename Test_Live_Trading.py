...
class TradingEnvironment:
    def __init__(self, data, starting_balance=20):
        ...
        self.correl = data['correl_delta'].values
    ...
    def step(self, action):
        ...
        if action == 1 and len(self.entry) == 0 and self.close_price[self.current_step] > self.open_price[
            self.current_step] and self.close_price[self.current_step - 1] > self.open_price[self.current_step - 1] and self.ema20[self.current_step - 1] < self.ema50[self.current_step - 1] and self.ema20[
            self.current_step] > self.ema50[self.current_step]:
            loss_pct = 0.03
            gain_pct = 0.15
        ...
        elif action == 1 and len(self.entry) == 0 and self.close_price[self.current_step] < self.open_price[
            self.current_step] and self.close_price[self.current_step - 1] < self.open_price[self.current_step - 1] and self.ema20[self.current_step - 1] > self.ema50[self.current_step - 1] and self.ema20[
            self.current_step] < self.ema50[self.current_step]:
            loss_pct = 0.03
            gain_pct = 0.15
...
def test_model(network, test_data):
    ...
    while True:
        ...
        def normalize_cor(field):
            global val
            if field.correl[field.current_step] > 0.1 and field.correl[field.current_step] < 0.2:
                val = 0.1
            elif field.correl[field.current_step] > 0.2 and field.correl[field.current_step] < 0.3:
                val = 0.2
            elif field.correl[field.current_step] > 0.3 and field.correl[field.current_step] < 0.4:
                val = 0.3
            elif field.correl[field.current_step] > 0.4 and field.correl[field.current_step] < 0.5:
                val = 0.4
            elif field.correl[field.current_step] > 0.5 and field.correl[field.current_step] < 0.6:
                val = 0.5
            elif field.correl[field.current_step] > 0.6 and field.correl[field.current_step] < 0.7:
                val = 0.6
            elif field.correl[field.current_step] > 0.7 and field.correl[field.current_step] < 0.8:
                val = 0.7
            elif field.correl[field.current_step] > 0.8 and field.correl[field.current_step] < 0.9:
                val = 0.8
            elif field.correl[field.current_step] > 0.9 and field.correl[field.current_step] < 1.0:
                val = 0.9
            elif field.correl[field.current_step] < -0.1 and field.correl[field.current_step] > -0.2:
                val = -0.1
            elif field.correl[field.current_step] < -0.2 and field.correl[field.current_step] > -0.3:
                val = -0.2
            elif field.correl[field.current_step] < -0.3 and field.correl[field.current_step] > -0.4:
                ...
                val = -0.9
            elif field.correl[field.current_step] == 1:
                val = 1.0
            elif field.correl[field.current_step] == -1:
                val = -1
            return val
        ...
        state = np.concatenate([[signal2()], [signal_symbolused_price(env, 0)], [signal_symbolused_price(env, 1)],
                                ...
                                [signal_symbolused_price(env, 14)], [normalize_cor(env)]])
...
if __name__ == "__main__":
    ...
    df_symbolused = pd.DataFrame({
        ...
        'high_symbolusedmark': [],
        'low_symbolusedmark': [],
        'close_symbolusedmark': [],
        'volume_symbolusedmark': [],
        'symbol3': [],
        'open_totalmark': [],
        'high_totalmark': [],
        'low_totalmark': [],
    ...
    # Rename columns for consistency
    df_symbolused_mark.columns = ['symbol2', 'open_symbolusedmark', 'high_symbolusedmark', 'low_symbolusedmark',
                                  'close_symbolusedmark', 'volume_symbolusedmark']
    df_total_mark.columns = ['symbol3', 'open_totalmark', 'high_totalmark', 'low_totalmark', 'close_totalmark',
                             'volume_totalmark']

    # Merge the datasets on index (align the timestamps)
    df_combined = pd.concat([df_symbol, df_symbolused_mark, df_total_mark], axis=1)
    df_combined['tickers_delta'] = df_combined['close_totalmark'] - df_combined['close_symbolusedmark']

    df_symbolused = pd.concat([df_symbolused, df_combined[:len(df_combined) - 2]], ignore_index=True)
        ...
        try:
            ...
            while logic[-1]:
                try:
                    ...
                    # Specify parameters
                    symbol_used1 = 'TRXUSDT.P'
                    platform = 'OKX'
                    n_bars1 = 500

                    # Fetch historical data for symbol_used
                    df_symbolused1 = fetch_data_with_retry(tv, symbol_used1, platform, Interval.in_5_minute, n_bars1)

                    if df_symbolused1 is None:
                        raise ValueError("Failed to fetch data for symbol_used.")

                    # Rename columns for consistency
                    df_symbolused1.columns = ['symbol', 'open_symbolused', 'high_symbolused', 'low_symbolused',
                                        'close_symbolused',
                                        'volume_symbolused']

                    # Symbols for additional data
                    symbolused_marketcap1 = 'TRX'
                    total_marketcap1 = 'TOTAL'
                    platform_cryptocap1 = 'CRYPTOCAP'

                    # Fetch OHLC data for SOL ,BTC and TOTAL
                    df_symbolused_mark1 = fetch_data_with_retry(tv, symbolused_marketcap1, platform_cryptocap1,Interval.in_5_minute, n_bars1)
                    df_total_mark1 = fetch_data_with_retry(tv, total_marketcap1, platform_cryptocap1, Interval.in_5_minute,n_bars1)

                    # Rename columns for consistency
                    df_symbolused_mark1.columns = ['symbol2', 'open_symbolusedmark', 'high_symbolusedmark',
                                                  'low_symbolusedmark', 'close_symbolusedmark', 'volume_symbolusedmark']
                    df_total_mark1.columns = ['symbol3', 'open_totalmark', 'high_totalmark', 'low_totalmark',
                                             'close_totalmark', 'volume_totalmark']

                    # Merge the datasets on index (align the timestamps)
                    df_added = pd.concat([df_symbolused1, df_symbolused_mark1, df_total_mark1], axis=1)

                    df_added['tickers_delta'] = df_added['close_totalmark'] - df_added['close_symbolusedmark']
                    df_added.dropna(inplace=True)  # Drop NaN values due to potential mismatched timestamps
            ...
            passphrase = "Enter_Passphrase_Here"
            ...
            if len(positions_recent) == 0:
                ...
                if data_inputs['close_symbolused'].iloc[-1]>data_inputs['open_symbolused'].iloc[-1] and data_inputs['close_symbolused'].iloc[-2]>data_inputs['open_symbolused'].iloc[-2]:  # Buy action
                    console.print('[yellow]Bullish Candle Detected[/yellow]')
                    ...
                    if actions[-1] == 1:
                        ...
                        console.print(
                            f'[yellow]Action: Buy (Bullish Candle). SL: {sl_price}. Entry: {entry_price}. TP: {take_profit_price}, Leverage: {leverage}, Position Size: {position_size_usdt}[/yellow]')
                ...
                elif data_inputs['close_symbolused'].iloc[-1]<data_inputs['open_symbolused'].iloc[-1] and data_inputs['close_symbolused'].iloc[-2]<data_inputs['open_symbolused'].iloc[-2]:  # Sell action
                    console.print('[yellow]Bearish Candle Detected[/yellow]')
                    ...
                    if actions[-1] == 1:
                        ...
                        set_leverage(symbol_2, leverage, marginmode)
                        console.print(
                            f'[yellow]Action: Sell (Bearish Candle). SL: {sl_price}. Entry: {entry_price}. TP: {take_profit_price}, Leverage: {leverage}, Position Size: {position_size_usdt}[/yellow]')
                    ...
                    ema20 = data_inputs['ema20'].iloc[-1]
                    ema50 = data_inputs['ema50'].iloc[-1]
                    console.print(
                        f'[green]Action: Hold. High: {prev_high}. Low: {prev_low}. Open: {prev_open}. Close: {prev_price}. EMA20: {ema20}. EMA50: {ema50}[/green]')
