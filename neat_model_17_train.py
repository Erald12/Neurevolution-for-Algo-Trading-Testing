...
while True:
    ...
    class TradingEnvironment:
        ...
        def step(self, action):
            ...
            if action == 1 and len(self.entry) == 0 and self.close_price[self.current_step]>self.open_price[self.current_step] and self.close_price[self.current_step-1]>self.open_price[self.current_step-1]:
                loss_pct = 0.03
                gain_pct = 0.15
            ...
            elif action == 1 and len(self.entry) == 0 and self.close_price[self.current_step]<self.open_price[self.current_step] and self.close_price[self.current_step-1]<self.open_price[self.current_step-1]:
                loss_pct = 0.03
                gain_pct = 0.15
    ...
    def run_neat(config_path, save_path="best_genome3.pkl", generations=999999999999):
        ...
        def evaluate_genomes(genomes, config, prune_every=10):
            ...
            for genome_id, genome in genomes:
                ...
                while True:
                    def signal2():
                        if env2.close_price[env2.current_step]>env2.open_price[env2.current_step] and env2.close_price[env2.current_step-1]>env2.open_price[env2.current_step-1]:
                            signal = 1
                        elif env2.close_price[env2.current_step]<env2.open_price[env2.current_step] and env2.close_price[env2.current_step-1]<env2.open_price[env2.current_step-1]:
                            signal = -1
                        else:
                            signal = 0
                        return signal
