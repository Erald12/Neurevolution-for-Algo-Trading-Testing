...
class TradingEnvironment:
    ...
    def step(self, action):
        ...
        if action == 1 and len(self.entry) == 0 and self.close_price[self.current_step] > self.open_price[
            self.current_step] and self.close_price[self.current_step - 1] > self.open_price[self.current_step - 1]:
            loss_pct = 0.03
            gain_pct = 0.15
...
def test_model(network, test_data):
    ...
    while True:
        ...
        def signal2():
            if env.close_price[env.current_step]>env.open_price[env.current_step] and env.close_price[env.current_step-1]>env.open_price[env.current_step-1]:
                signal = 1
            elif env.close_price[env.current_step]<env.open_price[env.current_step] and env.close_price[env.current_step-1]<env.open_price[env.current_step-1]:
                signal = -1
            else:
                signal = 0
            return signal
